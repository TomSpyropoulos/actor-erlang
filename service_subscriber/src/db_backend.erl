...
%% delegates its DB operations to one instance of the module DB_BACKEND selects. Buffering is NOT
%% part of this contract: the dispatcher (service_subscriber_db) owns the buffer, the BATCH_SIZE /
%% BATCH_TIMEOUT_MS triggers and the flush timer, and calls insert/5 or insert_batch/2 accordingly,
%% so no backend can reinterpret what the swept batch factors mean. To add a backend, implement all
%% six callbacks and add a clause in service_subscriber_db:resolve_backend/0.
-module(db_backend).
...
%% while db_backend_timescaledb takes array parameters and ignores it.
-callback init(Index :: non_neg_integer(),
               Opts  :: #{batch_enabled := boolean(), batch_size := pos_integer()}) ->
    {ok, BackendState :: term()}.

%% Submit one sensor-data row. Called only when batching is off. MsgTimestampUs is both the value
%% written to the Timestamp column and the start of the e2e latency measurement, and the backend
%% converts it to whatever its driver wants, so no DB-shaped type crosses this contract.
%% ProcStartUs only computes the subscriber->DB latency.
%%
%% Return {async, NewState} when the write was dispatched asynchronously: handle_result/2 is then
%% called for every later message until the backend claims the ack. Return {sync, ...} when the
%% write completed inline and measured the latency itself, with [] reporting a failed write.
