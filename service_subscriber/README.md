...
- **Root Supervisor (`service_subscriber_sup`)**: The core supervisor that monitors the metrics server, any processes the selected backend shares across its pool, database connection pool, dynamic worker supervisor, read-load generators, and MQTT receiver. Reader children are built only when `READS_PER_SEC > 0`, so the default configuration starts no reader processes and opens no reader connections.
...
- **DB Backend Behaviour (`db_backend`)**: An Erlang behaviour defining the pluggable interface for database backends. It declares six callbacks — `init/2`, `insert/5`, `insert_batch/2`, `insert_status/3`, `handle_result/2`, and `terminate/1` — that any backend must implement, plus an optional `child_specs/0` for processes every pool worker of one backend shares, which the root supervisor starts ahead of the pool. Only `db_backend_sqlite` declares one. `init/2` receives the dispatcher's batching decision as `#{batch_enabled => boolean(), batch_size => pos_integer()}` so a backend can skip preparing resources it will never use, and pre-build any whose shape depends on the size, without re-reading the environment itself. `batch_size` matters to a backend whose batch statement has fixed arity — a multi-row `VALUES` list, as in `db_backend_mysql` — and is ignored by one taking array parameters. `insert/5` (batching off) and `insert_batch/2` (batching on) take the reading's timestamp only as epoch microseconds and leave each backend to build whatever representation its driver binds, so no DB-shaped type crosses the contract. Buffering is deliberately **not** part of the contract — the dispatcher decides when a write happens, the backend decides only how. Both return either `{async, NewState}` for an asynchronously dispatched write or `{sync, Latencies, NewState}` when the write completed inline. `handle_result/2` returns `{match, [{E2EUs, SubToDbUs}], NewState}` with a list of latency pairs (one per acknowledged row) when it recognises the message, or `{no_match, NewState}` otherwise. The active backend is selected at startup via the `DB_BACKEND` environment variable, making it straightforward to swap or add backends without touching the dispatcher or worker layers.
...
- **MySQL Backend (`db_backend_mysql`)**: The concrete `db_backend` for MySQL 8.4 via mysql-otp. Statements are prepared once per worker at `init/2`; the batch statement is prepared only when batching is on, and is built for exactly `batch_size` rows, since a multi-row `VALUES` list has fixed arity where TimescaleDB's array parameters do not — a `BATCH_TIMEOUT_MS` flush on a partly filled buffer therefore falls back to a statement built for its own length. mysql-otp has **no async API**: `mysql:execute/3` is a `gen_server:call`. Both write paths therefore hand the blocking call to a short-lived `spawn_link`ed process that messages the ack back as `{mysql_ack, Ref, Result}`, so this backend still returns `{async, State}` and its acks still arrive through `handle_result/2` — the Erlang arm's execution model is the same for both databases. What the spawn does **not** recover is wire-level concurrency: the MySQL protocol has no pipelining, so one connection carries one query at a time and in-flight writes are capped at `DB_POOL_SIZE`. With `BATCH_ENABLED=false` that cap binds at the swept load and the subscriber builds an unbounded backlog; with batching on it keeps up, so `pool_*` and `batching_*` results are not comparable across databases.
- **InfluxDB Backend (`db_backend_influxdb`)**: The concrete `db_backend` for InfluxDB 2.7, and the only one that speaks HTTP rather than a binary wire protocol. It needs no driver: `inets` is already an application dependency, and `httpc` in async mode (`{sync, false}`) delivers `{http, {RequestId, Result}}` straight to the worker's mailbox, so `{async, State}` is satisfied with no helper process and `handle_result/2` correlates by request id exactly as the other two do. `init/2`'s `Opts` is ignored — line protocol has no fixed arity and there is no statement to prepare, so neither `batch_size` nor `batch_enabled` changes anything here. Each worker owns a named `httpc` profile at `max_sessions = 1` with a deep keep-alive queue, which is what keeps `DB_POOL_SIZE` the connection count rather than letting httpc open throwaway sockets. Because HTTP connects lazily, `init/2` blocks on a readiness probe so an unreachable database still crashes the worker the way a driver connect would. This module and `InfluxDBBackend.scala` together *are* this backend's schema: there is no `init.sql`. Writes are upserts keyed by timestamp, so two readings from one sensor in the same microsecond overwrite silently while both count as committed.
- **SQLite Backend (`db_backend_sqlite`)**: The concrete `db_backend` for SQLite, embedded through the `esqlite` NIF, so there is no database container: every pool worker opens its own connection to one file on the `sqlite-storage` volume. SQLite has no server to create a schema, so `open/0` applies the mounted `connection.sql` (per-connection settings, including `synchronous=FULL`, which the esqlite build would otherwise lower) and then the idempotent `init.sql` on every connection. It is the only backend that returns `{sync, ...}`. An esqlite handle cannot be used from several processes, which rules out `db_backend_mysql`'s spawned helper, so the pool worker runs each write itself and blocks until it returns, the way a Scala `BatchWriterActor` does. A batch is one `BEGIN IMMEDIATE` transaction of single-row steps, so one fsync per flush and no fixed arity. `handle_result/2` never matches anything. `init/2`'s `Opts` is ignored.
- **MongoDB Backend (`db_backend_mongodb`)**: The concrete `db_backend` for MongoDB 8.0, through mongodb-erlang's `mc_worker_api`, which is a git dependency because hex carries no maintained release. Each pool worker owns one `mc_worker` connection, opened and authenticated by `connect/0`, which the read backend also uses. The driver's calls block, so writes reuse `db_backend_mysql`'s shape: a short-lived `spawn_link`ed helper runs the insert and messages `{mongodb_ack, Ref, Result}` back, and `handle_result/2` claims it. The driver raises on a failed command rather than returning an error, so the helper catches the raise and acks it as a failure. Unlike mysql-otp, `mc_worker` pipelines: several helpers' commands can be queued on one socket at once, where the Java driver holds a connection per write. Every insert carries `{w: 1, j: true}`, so an ack means the journal fsync the SQL backends' commits mean. `Data` is a time-series collection whose time field is a BSON Date, so the reading's timestamp is stored floored to milliseconds. A batch is one ordered `insert` command, so no fixed arity. `init/2`'s `Opts` is ignored.
- **SQLite Write Lock (`db_backend_sqlite_lock`)**: One GenServer shared by every SQLite pool worker, started ahead of the pool through `child_specs/0`. SQLite allows one writer, and esqlite runs each call on a dirty IO scheduler, of which there are 10 by default. Writers left to wait for the file lock inside SQLite's busy handler each hold a scheduler while they sleep, so at a pool of 20 they starved the lock holder of one and the pool deadlocked. Every write, and every connection's setup, now takes this lock first, so writers wait in the Erlang mailbox instead. Callers are served in arrival order, and a holder that dies releases it through its monitor.
...
- **MySQL Read Backend (`db_read_backend_mysql`)**: The concrete `db_read_backend` for MySQL. One mysql-otp connection per reader, statement prepared at `init/1`. Here the driver's synchronous API is exactly what the contract wants — `read/1` is specified as blocking — so the spawned-helper indirection the write path needs is deliberately absent. The query is the MySQL spelling of the same bounded-window aggregate, using `NOW(6)` rather than `NOW()` to match the microsecond resolution of Postgres `now()`. Its bounded window is only cheap because `mysql/init/init.sql` indexes `Timestamp`: InnoDB has no equivalent of the chunk exclusion TimescaleDB gets from `create_hypertable`. The SQL-matching rule is **per backend** — this module and `MySQLReadTarget.scala` must match each other, not the TimescaleDB pair.
- **InfluxDB Read Backend (`db_read_backend_influxdb`)**: The concrete `db_read_backend` for InfluxDB. One `httpc` profile per reader, and the read is a synchronous POST, which is what the contract wants. The query is the Flux spelling of the same bounded-window aggregate: `group()` is required because Flux otherwise aggregates per series and would return one row per device where the SQL backends return one overall, and `reduce` computes count and sum in a single scan. The bounded window stays cheap without an index, since the TSM engine is already time-ordered. One Flux read costs far more than its SQL equivalent, so the `reads_*` scenarios saturate at a lower read rate here. Byte-identical to `InfluxReadTarget.scala`; the matching rule is per backend.
- **SQLite Read Backend (`db_read_backend_sqlite`)**: The concrete `db_read_backend` for SQLite. One connection per reader, opened through `db_backend_sqlite:open/0` so a reader cannot be configured apart from the writers, and it does not take the write lock after setup. Under WAL a read runs alongside the writer, but its `step` still needs one of the dirty IO schedulers the writers use. `Timestamp` holds epoch microseconds, so the bounded window is computed in that unit, and `init.sql` indexes it for the same reason MySQL's does. Byte-identical to the Scala arm's SQLite read query; the matching rule is per backend.
- **MongoDB Read Backend (`db_read_backend_mongodb`)**: The concrete `db_read_backend` for MongoDB. One connection per reader, opened through `db_backend_mongodb:connect/0`. The query is an aggregate pipeline held as JSON text, so both arms can hold the same bytes, and parsed once at init with OTP's `json` module. `$$NOW` keeps the 5-second window on the server clock, like `NOW(6)`. It goes out as one raw OP_MSG command whose reply carries the single result document, which avoids the cursor process `mc_worker_api:command/2` would start per read. The window scans every time-series bucket, so read cost may grow with the number of rows written. Byte-identical to the Scala arm's MongoDB pipeline; the matching rule is per backend.
...
- State changes are logged via `logger` and persisted to `sensor_status` in the selected database, but only when the status actually changes to avoid unnecessary writes.
